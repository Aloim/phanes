<!-- Phanes v3.2 (2026-07-21). Single-file bootstrap prompt.
...
"templates": { "version": "3.2", "source": "fetched" },
"capabilities": {
...
| `opus` (Opus 4.8) | `high`; `xhigh` for security- or money-critical review | **All Critics (every tier).** The terminal quality gate on every chain, a missed defect costs more than the review. |
| `opus` (Opus 4.8) | `high` | **The Orchestrator role** (engaged plan runs, v3.2): per-step tier triage, chain composition, effort-bridge dispatch, batch SS authorship. One wrong routing decision wastes an entire worker chain, which dwarfs the tier premium. Rides the session baseline in-session; never routinely CLI-spawned. |
| `opus` (Opus 4.8) | `xhigh` | Architect/designer, Synthesizer/Arbiter, high-ambiguity or cross-module planning, and any security- or monetary-system design. Reserve the deepest reasoning for the judgments everything else depends on. |
