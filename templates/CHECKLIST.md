<!-- DOC | The install checklist Phanes works through when it fetches the v3.2.1 script template library. -->
<!-- phanes-template v3.2.1 CHECKLIST -->

# Template install checklist (v3.2.1)
...
2. Every fetched file passes the sanity check before it lands: the stamp comment `# phanes-template v3.2.1 <name>` (or `rem phanes-template v3.2.1 <name>` for the `.cmd` shim) appears within the first two lines, after the shebang on POSIX or after `@echo off` in the `.cmd` shim. A 404 body or an HTML error page must never be written into `.phanes/scripts/`.
...
9. The `config.json` `templates` block is written: `{ "version": "3.2.1", "source": "fetched" }`, or `"generated"` when the fallback path produced the scripts.
