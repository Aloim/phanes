...
The pre-flight detects your platform first and runs the matching commands: bash on POSIX systems, PowerShell on Windows. It inventories what you already have installed, including MCP servers, plugins, skills, and slash commands, and holds that inventory so it can wire matching tools into the right agents later under least privilege. Nothing you installed is ever changed or removed. It also ensures the official `frontend-design` skill is present, since skills cost no context until they are invoked, so UI and frontend work runs with deliberate design guidance loaded. Finally, each run keeps a progress ledger on disk at `.phanes/run-progress`, so a session that dies or compacts mid-bootstrap resumes from the last completed phase instead of starting over blind.
...
- **api-monitor**, the sole writer of the generated API baseline in `.phanes/registry/`, which it runs after every structural change.
...
Current: **v2.6.1** (2026-07-17). This point release makes an update run reconcile an existing script library instead of fetching over it. A library that an earlier version generated in the project's own language, or one that carries a project specific safety behavior, is now preserved and verified against its specification rather than replaced by the generic templates, and `.phanes/config.json` records that choice as a new `preserved` provenance value. It builds on v2.6, which ships the scripts that do not depend on your language as tested templates in a `templates/` folder that the install fetches, pinned to the running version, instead of writing them fresh every time, and falls back to generating them when the fetch cannot happen so offline installs stay complete. Each fetched script finds the project by walking up to `.phanes/config.json` and uses only relative paths, which closes a real bug where the enforcement hooks were wired to the Phanes repository instead of the target project. See the version stamp at the top of `phanes.md` for the exact version. The full release history is in [`Changelog.md`](Changelog.md), and every superseded version is archived verbatim in [`older version/`](older%20version/).
