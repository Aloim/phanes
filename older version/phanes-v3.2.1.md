<!-- Phanes v3.2.1 (2026-07-26). Single-file bootstrap prompt.
...
> 5. Always conclude with the terminal quality gate, in this exact order: Synthesizer (only when parallel perspectives were used) → Critic → designated visual verifier (post-apply, UI-touching changes only) → close-verifier (T2/T3 structural changes) → primary. The final Critic audits the *consolidated* output, never raw perspectives.
...
> 13. **Spawn-grant exception (v3.2):** only `<projectSlug>-orchestrator` spawns worker chains; specialists spawn read-only scouts per rule 9; no other agent spawns agents, and no agent's handoff table may route to the orchestrator, it is invoked ONLY by the primary session at plan launch.
...
> * **Budget (chars, hook-surfaced):** soft limit **35,000**, crop trigger **40,000**, measured by `phanes register-check` (Phase 2.5 Step 4), printed by `hook-size-check` on every edit of a hot file, and mirrored into close-verifier's report as an observation.
...
"templates": { "version": "3.2.1", "source": "fetched" },
"capabilities": {
...
|--------------|--------|-----------|
| `haiku` (Haiku 4.5) |, (no effort dial) | Scouts, retrieval, formatting, indexing, archive-digest condensation (Phase 2 register mandate), mechanical transforms a script or test suite guards. **Not** the default for authored code, its higher defect rate turns the Critic into an iteration engine, and each Reflect loop (re-work + re-review) costs more than writing it right once. |
| `sonnet` (Sonnet 5) | `high` (drop to `medium` for routine or mechanical work) | **DEFAULT** for coding agents, Analyzers, Validators, Optimizers, and frontend/UI specialists. Best accuracy-per-token for implementation and analysis. |
| `opus` (Opus 5) | `high`; `xhigh` for security- or money-critical review | **All Critics (every tier).** The terminal quality gate on every chain, a missed defect costs more than the review. |
| `opus` (Opus 5) | `high` | **The Orchestrator role** (engaged plan runs, v3.2): per-step tier triage, chain composition, effort-bridge dispatch, batch SS authorship. One wrong routing decision wastes an entire worker chain, which dwarfs the tier premium. Rides the session baseline in-session; never routinely CLI-spawned. |
| `opus` (Opus 5) | `xhigh` | Architect/designer, Synthesizer/Arbiter, high-ambiguity or cross-module planning, and any security- or monetary-system design. Reserve the deepest reasoning for the judgments everything else depends on. |
...
**Batching (the Orchestrator self-decides).** 1 to 3 consecutive steps per spawn, hard cap 3, never across a phase boundary. A T3 or high-ambiguity step runs as a batch of 1; light consecutive T1/T2 steps may batch to 2 or 3. Early close-out is always legal, exceeding the cap never is: if mid-batch context grows heavy or a step fails, finish or abort the current step, write the batch SS covering what actually ran, and hand back.

**Per batch, the Orchestrator MUST:** (1) estimate the batch; (2) per step, run tier triage (T1/T2/T3) and gather only the context that tier permits; (3) compose each chain per the Chain Design Rules, Critic terminal, `close-verifier` for T2/T3 structural, visual verifier for UI; (4) dispatch workers in-session at the baseline, and CLI-spawn any archetype whose `effort:` frontmatter EXCEEDS the baseline per the Per-Agent Effort Delivery bridge above (workers aligned with or below the baseline, and haiku, always ride in-session, never the bridge), and collect every CLI spawn per the bridge's collection rules before closing the batch, a batch never returns with a bridge spawn still in flight; (5) run the Reflect loop on `fix_required` verdicts; (6) write ONE batch session summary with a subsection per executed step; (7) return the receipt below and despawn. Bounded Fan-Out (max 5 in flight) applies inside the Orchestrator.

**The bounded batch receipt (exact JSON, never narrative):**
...
**The generated agent definition is self-carrying.** `.claude/agents/<projectSlug>-orchestrator.md` MUST restate, in its own operating protocol: the spawn-grant exception ("this agent is the sole non-scout archetype permitted to spawn agents; its workers may spawn scouts only, nothing deeper"), the batching rules, the early close-out right, the effort-bridge duty, and the receipt schema above, so the rules are visible at the point of use, not only in §II and §III. Its `description` field MUST state it is invoked ONLY by the primary session at plan launch, never mid-chain, and no other agent's Next Task table may route to it. `tools:` grant: Task (agent spawning), Write (batch SS and own report only), Bash (effort bridge and `.phanes/scripts/`), plus read/search tools per least privilege.
...
```
    ...
    <!-- If this agent is close-verifier, the Report Body MUST be an independent verification record
         ...
         5) Applied-vs-approved reconciliation: does the applied diff match what the Critic approved?
            An undisclosed deviation is a reportable drift event.
