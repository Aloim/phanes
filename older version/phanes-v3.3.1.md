<!-- Phanes v3.3.1 (2026-07-27). Single-file bootstrap prompt.
...
**Grouped Batch Execution (v3.3.1): one chain over the batch, not one chain per step.** Three small steps run per-step pay a producer turn, a Critic turn, and an Executor turn three times over for work amounting to a few dozen lines. The review depth those repeats buy is real at T3 and largely ceremony at T1. Grouping collapses the repetition without removing a single gate: Agent Persistence already deleted the respawn tax on those repeats, and grouping deletes the repeats themselves. A grouped batch of 3 T1 steps runs 2 spawned turns instead of 6; a grouped batch of 3 T2 steps runs 4 instead of 12.

* **The gate, decided by the Orchestrator at triage.** A batch runs **grouped** only when ALL of these hold: (1) every step is T1 or T2, any T3 step forces per-step; (2) no step is security-triggering, those fire elevation trigger (a) and the Security Review Gate is one pass per step by design, they earn isolated attention; (3) the steps' working sets do not overlap, no step edits a file another step in the batch edits, and no step consumes another step's output. Anything else runs **per-step**, exactly as before v3.3.1. Per-step is the default and grouping is the exception the gate has to earn; **ambiguity resolves to per-step.**
* **The overlap test is judgment, not arithmetic.** The Orchestrator already holds every step body, tier triage cannot be performed without reading them, and T1 is itself defined by a scope test of the same kind (single file, no exported API surface, Phase 2.5 Step 3). Judge from the step bodies. Where the plan declares per-step file sets, use them; where it does not, a step whose scope you cannot bound is not groupable.
* **What collapses.** One producer turn, one Critic turn, one Executor turn for the whole batch instead of one of each per step. `close-verifier` runs ONCE at batch close on T2 structural work, one `phanes regen-registry` and one `phanes api-diff` rather than one pair per step, which is better verification for less spend, not a weaker one. The designated visual verifier runs once over the union of the batch's declared viewports and screens. Effort resolution, the four elevation triggers, the budget of 2, and Bounded Fan-Out are untouched and still reasoned per step.
...
* **The Executor contract.** In grouped mode the Executor applies the batch's approved diffs in plan order with a checkpoint per step; on failure it stops at the failing step, leaves already-applied steps applied, and names that step id. `failed_step` in the receipt therefore stays exact and §III rule 12's failure handling needs nothing new.
* **Reflect over the failing subset.** On a mixed verdict, resume the producer ONCE carrying the findings for every failing step, then resume the Critic ONCE to re-review that subset only. Steps that passed stay applied and are not re-reviewed: the non-overlap gate is precisely what makes that safe, a fix to one step cannot reach a sibling's files. The grouped rework counts as ONE loop against the bounded Reflect cap, never one loop per failing step.
* **The record.** The batch SS carries the mode line below whether the batch grouped or not, naming the gate condition that failed when it did not. A mode decision leaving no trace is indistinguishable from the mechanism not working, the same reason the sizing and effort lines are mandatory.
...
* **Batch SS record lines (mandatory, the tuning records; the receipt carries none of this):**
  ...
  `Batch mode: grouped | per-step (<gate condition that failed>), steps <ids>, chain turns <N>.`
