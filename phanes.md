...
* **`phanes register-check`**, measures the two **hot files** (project root `CLAUDE.md` and `CLAUDE.local.md`) in characters and prints one status line per file: `OK` (below 35,000), `SOFT-BREACH` (35,000 to 40,000), or `CROP-REQUIRED` (above 40,000). Additionally lists every `## ✅` entry still present in the register (completed entries must be archived in their close-out change set, Phase 2 register mandate) and reports the 🛑 standing-blocker section's character count separately (the crop-exemption class must be measured to be challengeable). Advisory, always exits 0. Consumed by `hook-size-check`, the Cropping Operation, and close-verifier's report.
...
"templates": { "version": "3.0", "source": "fetched" },
"capabilities": {
