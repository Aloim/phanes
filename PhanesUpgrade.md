<!-- PhanesUpgrade v3.3.1, 2026-07-27, production upgrade prompt for existing Phanes installations.
...
| `.claude/workflows/*`, `.claude/template/*` (`report.md`; `agent-definition.md` from v3.3, both fetched by the regeneration run) | **REGENERATE** | |
...
7. **Frontmatter migration sweep (target v3.3+).** In every Phanes-prefixed `.claude/agents/<projectSlug>-*.md`, replace any absolute `effort: <value>` line with `effort_class: baseline` (delete the line entirely for haiku agents, which have no dial). Every absolute value maps to baseline, including xhigh; nothing ships elevated, `elevated` is a user lever. Foreign (user-authored) agents are untouched, and PRESERVE-and-flag agents are migrated only with the user, item by item, like every other change to them. A definition left carrying an absolute `effort:` is one the orchestrator can no longer resolve; this sweep is a correctness requirement, not a cleanup.
8. **Documentation system pass**, governed by phanes.md Phase 2.5 Step 2b:
...
9. **Manifest rewrite.** Write the new `.phanes/manifest.json` reflecting exactly what now exists (schema per the target spec's close-out rules: `{manifestVersion: 1, phanesVersion, stampedAt, projectSlug, artifacts: [{path, class, sha256, customized}]}`).
...
9. **Batch renegotiation protocol restated (target v3.3+).** Where `.claude/agents/<projectSlug>-orchestrator.md` exists, its operating protocol restates the renegotiation contract: the look-ahead block it must attach to the first planning-class agent's spawn prompt, adopt-by-default, the clamps (five from v3.3.1, the fifth being the grouped-batch clamp), and the mandatory batch-sizing line in the batch session summary. Also confirm the architect/designer agent carries the `batch_recommendation` emit duty and the prohibition on modifying the plan file. Either missing reopens the agent regeneration item.
10. **Effort model migrated (target v3.3+).** `grep -n "^effort:" .claude/agents/<projectSlug>-*.md` (PowerShell: `Select-String -Path .claude/agents/<projectSlug>-*.md -Pattern "^effort:"`). Required result: zero hits; every non-haiku prefixed definition carries `effort_class: baseline | elevated` instead. Foreign agents are excluded from the sweep.
11. **Persistence and effort protocols restated (target v3.3+).** The orchestrator agent's operating protocol restates the Agent Persistence protocol (scope table, resume match rule, recycle triggers with the retire notice, handle-table duty, termination at batch close) and the effort resolution protocol (the resolution table, the `+1`-only rule, the declared-baseline contract with its assume-`medium` fallback, the four elevation triggers, the consider-decline-record rule with the disposition-per-fired-trigger SS effort line, the budget of 2), and its `tools:` includes the agent-continuation affordance. The Critic agent carries the five self-fix bounds and a scoped Edit grant. Every agent whose duties include repo-wide enumeration (conformance auditors, Validator specializations, pattern and dead-code sweeps) is granted `semble` and carries the enumeration trigger in its MCP Usage Rubric; a `semble`-less auditor reverts to the Grep sweep the rubric exists to prevent. The register and project `CLAUDE.md` budget lines carry the trim target (20,000). Any missing piece reopens the corresponding regeneration item.
12. **Security Review Gate applied (target v3.3+).** Only where the roster carries a security-review agent; a roster without one passes this item vacuously. That agent's operating protocol states it is a serial, single-shot gate dispatched **once per step and only after the Code Critic returns `pass`**, carries the widened self-fix class (the Critic's trivial class plus single-site local security corrections, soft cap 10 changed lines, hard cap 20, mechanical check, diff attached), and states that a fix inside those bounds is **terminal** (no Critic re-review) while findings above them end its involvement in the step (Reflect loop plus Critic verification, the security reviewer never re-dispatched or resumed within the same step). No chain in `.claude/workflows/*.yaml` and no `handoffs[]` edge places it in the same parallel stage as the Critic; the project `CLAUDE.md` workflow rules carry the new rule 13 with the spawn-grant exception renumbered to 14, and rule 5's terminal-gate order names the security reviewer after the Critic. The `close-verifier` agent treats an attached post-Critic security diff as approved-with-attached-diff, not drift. Any missing piece reopens the corresponding regeneration item.
13. **Grouped batch execution present (target v3.3.1+).** Where `.claude/agents/<projectSlug>-orchestrator.md` exists, its operating protocol restates the grouped-batch protocol: the three gate conditions (every step T1 or T2, no security-triggering step, non-overlapping working sets), the rule that ambiguity resolves to per-step, per-step Critic verdicts keyed by step id, per-step Executor checkpointing with an exact `failed_step`, Reflect over the failing subset only, and the mandatory `Batch mode:` line in the batch session summary. Confirm too that the Critic agent carries the per-step verdict duty for grouped assignments and the Executor agent the ordered per-step checkpoint duty. Any missing piece reopens the agent regeneration item.
...
15. **Counter and sign-off.** Increment `.claude/.phanes`. Present the upgrade branch for user review, the **user** merges; you do not. Close verbatim (do not paraphrase):
