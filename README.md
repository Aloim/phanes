...
**Recommended model Sonnet 5, recommended effort `high`.** Launch with `claude --model sonnet --effort high`. Effort is set once at session launch and governs the session and everything it spawns, so this one dial is the whole setting. `high` is now a single fixed level, not a baseline to lift: every agent in the roster runs at it, and the only variation left is model, Opus 5 for two named exceptions, the plan-authoring chain (the architect and its reviewing Critic, through plan close) and the security-review specialization, Sonnet 5 for everything else. See [How to use](#how-to-use) for the details.
...
```text
     ...
     **** Grouped batches. Where every step is T1 or T2, none touches security,
          and none edits a file another one edits, the batch runs as one chain
          rather than one per step, so three small steps cost one review pass
          instead of three. Nothing is skipped: the Critic still returns its two
          verdicts per step, the Executor still applies step by step so a failure
          names the exact step, and anything ambiguous stays per-step.
```
...
- **Start the session on Sonnet 5 at `high` reasoning effort.** Reasoning effort is set once, when the session launches, and it governs the primary session and every sub-agent it spawns in that session. Launch with `claude --model sonnet --effort high` (or export `CLAUDE_CODE_EFFORT_LEVEL=high` before starting). **Sonnet 5 at `high` is the recommended base for Phanes from v3.4 on**, because effort is a single fixed level now, not a per-agent dial: every roster agent runs at `high`, and the only variation left is which model, Opus 5 for the plan-authoring chain (the architect and its reviewing Critic, through plan close) and for the security-review specialization, Sonnet 5 for everything else. `xhigh` is retired; there is no hotter setting to reach for. Set effort at launch rather than mid-run: changing it mid-session writes to your global settings and leaks into other projects and parallel sessions.
